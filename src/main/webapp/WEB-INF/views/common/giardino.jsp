			...
			<div class="griglia-giardino">
				<c:forEach var="item" items="${giardino}">
					<div class="casella-albero">
						<img src="${pageContext.request.contextPath}/Immagini/${item.albero.pathImmagine}" alt="${item.albero.nome}">
						<h3>${item.albero.nome}</h3>
						<p>Ne possiedi: <strong>${item.quantita}</strong></p>
						
						<form action="${pageContext.request.contextPath}/common/SpedizioneFrutti" method="post">
							<input type="hidden" name="idAlbero" value="${item.albero.idAlbero}">
